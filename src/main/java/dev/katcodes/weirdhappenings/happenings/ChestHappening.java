...
public class ChestHappening extends HappeningBase {
  ...
  public boolean runHappening(Player player) {
    BlockPos chestPos =
        BlockUtils.FindBlock(player, 10, ChestBlock.class, false);
    if (chestPos != null) {

      ChestBlock chestBlock =
          (ChestBlock)player.level.getBlockState(chestPos).getBlock();
      if (player.level.getBlockEntity(chestPos) instanceof ChestBlockEntity) {
        ChestBlockEntity entity =
            (ChestBlockEntity)player.level.getBlockEntity(chestPos);
