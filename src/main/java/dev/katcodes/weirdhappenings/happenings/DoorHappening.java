    ...
    public boolean runHappening(Player player) {
        ...
        if(door!=null) {
           ...
           player.level.setBlock(door, doorState, 10);
            player.level.levelEvent(player, doorState.getValue(OPEN) ? this.getOpenSound() : this.getCloseSound(), door, 0);
            player.level.gameEvent(player, doorBlock.isOpen(doorState) ? GameEvent.BLOCK_OPEN : GameEvent.BLOCK_CLOSE, door);
        }
        return true;
    }
