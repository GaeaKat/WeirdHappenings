  ...
  public boolean runHappening(Player player) {
    ...
    if (torch != null) {
      ...
      while (attempts < 50) {
        attempts++;
        for (BlockPos newPos :
             BlockPos.randomInCube(new Random(), 1, torch, 3)) {
          if (player.level.getBlockState(newPos).isAir() &&
              Blocks.TORCH.canSurvive(player.level.getBlockState(torch),
                                      player.level, newPos)) {
            player.level.setBlock(newPos, Blocks.TORCH.defaultBlockState(),
                                  Constants.BlockFlags.DEFAULT_AND_RERENDER);
            player.level.setBlock(torch, Blocks.AIR.defaultBlockState(),
                                  Constants.BlockFlags.DEFAULT_AND_RERENDER);
            return true;
          }
        }
