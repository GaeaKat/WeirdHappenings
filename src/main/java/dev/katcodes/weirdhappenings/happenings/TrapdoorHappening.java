...
public class TrapdoorHappening extends HappeningBase {
  ...
  public boolean runHappening(Player player) {
    BlockPos door =
        BlockUtils.FindBlock(player, 10, TrapDoorBlock.class, false);
    if (door != null) {
      ...
      if (doorState.getValue(WATERLOGGED)) {
        player.level.getLiquidTicks().scheduleTick(
            door, Fluids.WATER, Fluids.WATER.getTickDelay(player.level));
      }
  ...
  protected void playSound(@Nullable Player p_57528_, Level p_57529_,
                           BlockPos p_57530_, boolean p_57531_) {
    if (p_57531_) {
      int i = 1007;
    ...
    p_57529_.gameEvent(p_57528_,
                       p_57531_ ? GameEvent.BLOCK_OPEN : GameEvent.BLOCK_CLOSE,
                       p_57530_);
  }
